...
public class TienLenMienNam {
	...
	private Card minCard = new CardofTienLen();// đã thêm-là lá bài bé nhất, 4 players -> là 3 bích
	...
	public void newMotherPack() {
		...
		for (String suit : suits) {
			for (String value : values) {
				motherPack.addCard(new CardofTienLen(value, suit));
			}
		}
...
//		motherPack.printDeck();
	...
	public void dealCard() {
		...
		for (int i = 0; i < numOfAIPlayer; i++) {
			AiPlayer playerTemp = new AiPlayer();
			for (int j = 0; j < 13; j++) {
				playerTemp.addCard(motherPack.removeCard(0));
			}
			playerTemp.sortDeck();
		...
		if (selectionCard.size() > 0) {
			...
			if (lastPlayType == PlayType.NULL) {// đã sửa ở đây
				...
				if (selectionCardType == PlayType.TU && lastPlayType == PlayType.DOI_THONG && b + 1 == 6) {
					...
					key = 0;
					return true;
				} else if (lastPlay.getCard(b).getCardValue() == 15) {
					if (b == 0) {
	...
	public boolean isTheEnd() {
		return rank.size() == numOfPlayer; // Kiểm tra đầy đủ số lượng người chơi
	}
	...
	public boolean checkDoi(Card card, Card card2) {
	...
	public PlayType checkSanh(Deck deck) {
		deck.sortDeck();
		if (deck.size() == 0 || deck.getCard(deck.size() - 1).getCardValue() == 15)
			return PlayType.NULL;
		if (deck.size() < 3) {
			return PlayType.NULL;
		}
		for (int i = 0; i < deck.size() - 1; i++) {
			if ((deck.getCard(i + 1).getCardValue() - deck.getCard(i).getCardValue()) != 1) {
				return PlayType.NULL;
			}
		}
